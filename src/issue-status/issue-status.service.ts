...
export class IssueStatusService {
  ...
  readAll(skipNumber = 0, takeNumber = 20): Observable<IssueStatus[]> {
    return from(this.issueStatusRepository.find()).pipe(
      skip(skipNumber),
      take(takeNumber),
    );
  }
  ...
  readOne(id: number): Observable<IssueStatus> {
    return from(this.issueStatusRepository.findOne(id));
  }

  update(issueStatus: IssueStatus): Observable<UpdateResult> {
    return from(this.issueStatusRepository.update(issueStatus.id, issueStatus));
  }
